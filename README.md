...
The objective of this assignment is to determine the unknown parameters $\theta$, $M$, and $X$ of a given parametric curve using the provided $x,y$ data points.
...
The goal is to determine the values of $\theta$, $M$, and $X$ that make the reconstructed curve match the provided data as closely as possible.
...
For the correct values of $\theta$, $M$, and $X$, the calculated value of $A$ should match:
...
7. Used `scipy.optimize.least_squares` to estimate $\theta$, $M$, and $X$.
...
| Metric | Result |
|---|---:|
| Maximum X error | $\approx 2.03 \times 10^{-5}$ |
| Maximum Y error | $\approx 3.51 \times 10^{-5}$ |
| Mean L1 error | $\approx 2.06 \times 10^{-5}$ |
| Total L1 error | $\approx 0.03083$ |
